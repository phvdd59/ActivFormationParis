...
public class ExoAlgo1 implements ImpAlgo1 {
	...
	// cet algo fait toutes les itérations et renvoit le dernier point
	// d'équlibre calculé
	// il renvoit -1 s'il n'y a pas de point d'équilibre
	public int solution2(int[] tab) {

		int result = -1;
		for (int j = 0; j < tab.length; j++) { // on teste toutes les positions
												// j que peut avoir le point
												// d'équilibre (entre 0 et
												// tab.length-1)
			long somme1 = 0; // le but de l'algo est de determiner la somme des
								// elements à gauche du point d'équilibre
								// (somme1)
			long somme2 = 0; // et la somme des éléments à droite (somme2), afin
								// de les comparer

			for (int i = 0; i < j; i++) { // calcul de somme1 = somme des
											// éléments à gauche de j
				somme1 += (long) tab[i]; // ce sont les éléments de tab de 0 à
											// j-1
			} // si j=0, cette boucle est ignorée car i=0 < j=0 est bien faux
				// Donc somme1 ne change pas et =0

			for (int i = j + 1; i < tab.length; i++) { // calcul de somme2 =
														// somme des éléments à
														// droite de j
				somme2 += (long) tab[i]; // ce sont les éléments de j+1 à
											// tab.length-1
			} // si j=tab.length-1, cette boucle est ignorée car i=tab.length <
				// tab.length est bien faux
				// Donc somme2 ne change pas et =0

			if (somme1 == somme2) { // si somme1 = somme2 alors j est un point
									// d'équilibre
				result = j; // il faut noter que cet algo écrase result à chaque
							// fois qu'un point d'équilibre est trouvé
			} // par conséquent, result aura la valeur du dernier point
				// d'équilibre calculé
		}
		return result;
	}
	...
	public int solution(int[] tab) {
		...
		if (tab != null) {
			...
			while (j < tab.length && trouve == false) { // principale difference
														// avec l'algo précédent
														// :
														// on boucle tant qu'on
														// a pas trouvé
				long somme1 = 0; // et tant qu'on a pas testé toutes les
									// positions possibles de j
				long somme2 = 0;

				for (int i = 0; i < j; i++) { // aucun changement pour les
												// calculs de somme1 et somme2
					somme1 += tab[i];
				}
				...
				if (somme1 == somme2) {
					result = j; // dés qu'on trouve un point d'équilibre, on
								// passe le boolean "trouve" à true
					trouve = true; // on sort donc de la boucle
				}
				j++; // TRES IMPORTANT : ne pas oublier d'incrementer j à la fin
						// de l'algo
